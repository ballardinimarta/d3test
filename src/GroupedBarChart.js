...
function GroupedBarChart(
  ...
  console.log(width - marginLeft - marginRight + 6);
  //Background
  ...
  const TAxis = svg
    ...
    .call((g) =>
      g
        ...
        .attr("transform", `translate(${cellSize + marginLeft + 2}, 0)`)
    )
    .call((g) =>
      g
        .select(".tick:nth-of-type(2)")
        .attr("transform", `translate(${(cellSize + 1) * 3 + marginLeft}, 0)`)
    )
    .call((g) =>
      g
        .select(".tick:last-of-type")
        .attr("transform", `translate(${(cellSize + 1) * 5.5 + marginLeft}, 0)`)

        .select("rect")
        .attr("width", cellSize * 3 + 8)
        .attr("x", -cellSize * 1.5)
    )
    .call((g) => g.selectAll(".tick text").remove())
    .call((g) =>
